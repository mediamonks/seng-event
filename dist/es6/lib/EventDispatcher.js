...
export default class EventDispatcher extends Disposable {
     ...
     */
    addEventListener(eventType, handler, useCapture = false, priority = 0) {
        ...
        // end todo
        const data = new EventListenerData(this, eventType, handler, useCapture, priority);
     ...
     */
    hasEventListener(eventType, handler, useCapture) {
     ...
     */
    removeEventListener(eventType, handler, useCapture = false) {
 ...
 */
export const removeListenersFrom = (listeners, eventType, handler, useCapture) => {
...
export const callListeners = (listeners, event) => {
    ...
    for (let i = 0; i < listenersOfType.length; i++) {
        ...
        if (event.eventPhase !== disabledOnPhase && !listenersOfType[i].isRemoved) {
            const callResult = event.callListener(listenersOfType[i].handler);
